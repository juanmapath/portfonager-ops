...
from apps.gemsfinder.funcs.scrappers import (
    scrap_finviz_screener, scrap_finviz_screener_costum,
    update_finviz_metrics_series, get_selenium_driver
)
...
def compare_industry(industry, driver=None):
    industry_filter_parse = industry.lower().replace(" ","").replace("&","").replace("-","")
    filters_list = {"Industry": industry_filter_parse}

    df_compare = scrap_finviz_screener_costum(filters_list, top=True, driver=driver)
...
def run_st():
    ...
    # Open a single Selenium driver for the entire session to avoid
    # repeated browser startup overhead and to share cookies/session with Finviz.
    driver = get_selenium_driver()
    print("[GemsFinder] Selenium driver started.")
    ...
    try:
        for tactic in tactics:
            ...
            asset_df = scrap_finviz_screener_costum(params, driver=driver)
            ...
            for industry in unique_industries:
                ...
                compare_asset_df = compare_industry(industry, driver=driver)
            ...
            for idx, row in asset_df.iterrows():
                ...
                success, merged_data = update_finviz_metrics_series(ticker, driver=driver)
        ...
        try:
            driver.quit()
            print("[GemsFinder] Selenium driver closed.")
        except Exception:
            pass
...
if __name__ == "__main__":
    # Setup Django when running as a standalone script (not via Django-Q worker)
    import django
    from pathlib import Path
